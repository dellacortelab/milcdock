...
from ensemble_dock.dataset import DockingDataset, dataset_dict
from ensemble_dock.evaluate import calc_auc, calc_bedroc, calc_ef, binary_acc
...
def make_fig_3(args, results_dfs):
    ...
    for model_name in args.model_name:
        eval_output = get_metrics(model_name, results_dfs[args.dataset[0]])
        results = pd.concat((results, pd.Series(eval_output['all_t_ef'], name=model_name)), axis=1)

    save_df_as_image(results, path=os.path.join(args.figs_dir, args.dataset[0] + ".svg"))
...
def save_df_as_image(df, path):
    ...
    sns.heatmap(df, annot=True, cmap=cmap, cbar=False, fmt='.2f')
    fig.tight_layout(pad=2)
    fig.savefig(path)
