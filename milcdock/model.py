...
def get_saved_model(model_path, device):
    if os.path.isdir(model_path):
        ...
        for model_load_path in os.listdir(model_path):
            ...
            model = DockingClassifier(depth=model_settings.depth, layer_4_dim=model_settings.layer_4_dim, activation=model_settings.activation, dropout=model_settings.dropout, dock_tools=model_settings.dock_tools)
...
class DockingClassifier(nn.Module):
    def __init__(self, output_dim=1, depth=90, layer_4_dim=64, width=64, activation='relu', dropout=.4, dock_tools=['vina', 'autodock', 'plants', 'ledock', 'rdock'], train_dataset_statistics=None):
        super().__init__()
        ...
        if train_dataset_statistics is None:
            train_dataset_statistics = {'mean':torch.zeros(input_vec_len), 'std':torch.ones(input_vec_len)}
    ...
    def forward(self, inputs):
        z_scores = (inputs - self.train_data_mean) / self.train_data_std
        x = self.layer_1(z_scores)
